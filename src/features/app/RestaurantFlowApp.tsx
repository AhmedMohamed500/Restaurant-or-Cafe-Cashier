...
function InventoryView({ data }: { data: AppData }) {
  const warehouse = stageWarehouse(data, "raw"); const items = data.items.filter((item) => item.stage !== "finished" && item.active);
  ...
  return <><div className="stats"><Stat label="مواد المخزون" value={String(items.length)} hint="مواد مسجلة" icon="◇" /><Stat label="قيمة المخزون" value={formatMoney(value)} hint="بمتوسط التكلفة" icon="ج" /><Stat label="أذون الإضافة" value={String(receipts.length)} hint="حركات موثقة" icon="＋" /><Stat label="تحت الحد الأدنى" value={String(items.filter((item) => (balanceFor(data, warehouse?.id, item.id)?.quantity ?? 0) <= item.minLevel).length)} hint="تحتاج متابعة" icon="!" /></div><div className="panel"><div className="panel-head"><div><h2>رصيد المخزن الرئيسي</h2><small>لا يمكن تعديل الرصيد مباشرة</small></div><span className="badge blue">{warehouse?.nameAr}</span></div>{items.length ? <div className="table-wrap"><table><thead><tr><th>المادة</th><th>الكود</th><th>التصنيف</th><th>الرصيد</th><th>متوسط التكلفة</th><th>القيمة</th><th>الحالة</th></tr></thead><tbody>{items.map((item) => { const balance = balanceFor(data, warehouse?.id, item.id); const quantity = balance?.quantity ?? 0; return <tr key={item.id}><td><div className="item-with-image"><ItemImage item={item} fallback="🥬" /><div className="item-name"><strong>{item.nameAr}</strong><small>{item.nameEn}</small></div></div></td><td>{item.code}</td><td>{item.category}</td><td>{displayQuantity(data, item, quantity)}</td><td>{formatMoney(balance?.averageCostPiasters ?? item.averageCostPiasters)}</td><td>{formatMoney(quantity * (balance?.averageCostPiasters ?? 0))}</td><td><span className={`badge ${quantity <= item.minLevel ? "amber" : "green"}`}>{quantity <= item.minLevel ? "منخفض" : "متاح"}</span></td></tr>; })}</tbody></table></div> : <Empty icon="1" title="المخزون فارغ" text="نفّذ أول إذن إضافة وسجّل المادة والكمية والوحدة والتكلفة." />}</div><MovementLedger data={data} movements={receipts.slice(0, 8)} title="آخر أذون الإضافة" /></>;
}
...
function MovementLedger({ data, movements, title }: { data: AppData; movements: StockMovement[]; title: string }) {
  if (!movements.length) return null;
  return <div className="panel ledger"><div className="panel-head"><div><h2>{title}</h2><small>سجل للقراءة فقط</small></div><span className="badge">{movements.length} حركة</span></div><div className="table-wrap"><table><thead><tr><th>المرجع</th><th>الحركة</th><th>الصنف</th><th>الكمية المدخلة</th><th>القيمة</th><th>التاريخ</th></tr></thead><tbody>{movements.map((movement) => { const item = data.items.find((entry) => entry.id === movement.itemId); return <tr key={movement.id}><td><strong>{movement.reference}</strong></td><td><span className={`badge ${movement.quantity > 0 ? "green" : "amber"}`}>{movementLabels[movement.type]}</span></td><td><div className="item-with-image"><ItemImage item={item} fallback="◇" /><strong>{item?.nameAr}</strong></div></td><td>{formatQuantity(movement.enteredQuantity ?? Math.abs(movement.quantity))} {data.units.find((unit) => unit.id === movement.enteredUnitId)?.symbol ?? data.units.find((unit) => unit.id === item?.baseUnitId)?.symbol}</td><td>{formatMoney(Math.abs(movement.totalCostPiasters))}</td><td>{new Date(movement.createdAt).toLocaleString("ar-EG", { dateStyle: "short", timeStyle: "short" })}</td></tr>; })}</tbody></table></div></div>;
}

function PosView({ data, cart, setCart, payment, setPayment, refresh, notify }: { data: AppData; cart: OrderItem[]; setCart: React.Dispatch<React.SetStateAction<OrderItem[]>>; payment: "cash" | "card" | "wallet"; setPayment: (value: "cash" | "card" | "wallet") => void; refresh: () => Promise<void>; notify: (text: string, error?: boolean) => void }) {
